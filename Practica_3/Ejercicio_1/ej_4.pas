...
procedure eliminar(var arch: archivo; n: integer);
var
	e, aux: empleado;
	ok: boolean;
begin
	reset(arch);
	ok:= false;
	seek(arch, filesize(arch) - 1); // Me posiciono en la ultima pos
	read(arch, aux); // Obtengo el elemento en la ultima pos
	seek(arch, 0); // Reseteo el index
	while(not eof(arch)) and (not ok) do begin
		read(arch, e); // Leo el registro actual
		if(e.nro = n) then begin
			ok:= true;
			seek(arch, filepos(arch) - 1);
			write(arch, aux); // Sobreescribo el registro buscado con el ultimo registro del archivo
			seek(arch, filesize(arch)-1); // Me posiciono en la ultima pos 
			truncate(arch); // Elimino el registro para evitar duplicados
		end;
	end;
	if (ok = false) then 
